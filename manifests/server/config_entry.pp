...
define postgresql::server::config_entry (
  ...
  if $facts['os']['name'] == 'Debian' or $facts['os']['name'] == 'Ubuntu' {
    if $name == 'data_directory' {
      $stop_command = ['service', shell_escape($postgresql::server::service_name), 'stop']
      $stop_onlyif = ['service', shell_escape($postgresql::server::service_name), 'status']
      $stop_unless = ['grep "data_directory =', join(["'", shell_escape($value), "'\""], ''), $postgresql::server::postgresql_conf_path]
    ...
    if $name == 'port' {
      ...
      $stop_command = ['service', shell_escape($postgresql::server::service_name), 'stop']
      $stop_onlyif = ['service', shell_escape($postgresql::server::service_name), 'status']
      $stop_unless = ['grep', join(["'PGPORT=", shell_escape($value), "'"], ''), '/etc/sysconfig/pgsql/postgresql']
      exec { "postgresql_stop_${name}":
        command => $stop_command,
        onlyif  => $stop_onlyif,
        unless  => $stop_unless,
      ...
      $stop_command = ['service', shell_escape($postgresql::server::service_name), 'stop']
      $stop_onlyif = ['service', shell_escape($postgresql::server::service_name), 'status']
      $stop_unless = ['grep', join(["'PGDATA=", shell_escape($value), "'"], ''), '/etc/sysconfig/pgsql/postgresql']
