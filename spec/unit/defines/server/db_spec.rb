...
describe 'postgresql::server::db', :type => :define do
  ...
  context "removal" do
    ...
    it { is_expected.to contain_postgresql__server__role('test').that_requires('Postgresql::Server::Database[test]').with_ensure('absent') }
    it { is_expected.not_to contain_postgresql__server__database_grant('GRANT test - ALL - test').with_ensure('absent') }
  end
end
